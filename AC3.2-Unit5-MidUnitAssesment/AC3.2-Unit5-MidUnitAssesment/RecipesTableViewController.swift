...
class RecipesTableViewController: UITableViewController, CellTitled, NSFetchedResultsControllerDelegate, UISearchBarDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tableView.register(UINib(nibName: "TableViewCell", bundle: nil), forCellReuseIdentifier: "recipeCell")
            ...
            if let validData = data {
                if let jsonData = try? JSONSerialization.jsonObject(with: validData, options:[]) {
                    if let wholeDict = jsonData as? [String:Any],
                        let recipes = wholeDict["results"] as? [[String:Any]] {
                        ...
                        pc.performBackgroundTask { (context: NSManagedObjectContext) in
                            ...
                            for recipe in recipes {
                                let object = NSEntityDescription.insertNewObject(forEntityName: "Recipe", into: context) as! Recipe
                                object.populate(from: recipe)
                            }
                            do {
                                try context.save()
                            }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
     ...
     let cell = tableView.dequeueReusableCell(withIdentifier: "recipeCell", for: indexPath) as! TableViewCell
     let recipe = fetchedResultsController.object(at: indexPath)
     
        cell.titleLabel.text = recipe.title
        cell.ingredientsLabel.text = recipe.ingredients
        cell.urlLabel.text = recipe.thumbnail
     return cell
     }
    ...
    func initializeFetchedResultsController(searchTerm: String? = nil) {
        ...
        let sort = NSSortDescriptor(key: #keyPath(Recipe.title), ascending: true)
